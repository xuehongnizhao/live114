...
@interface CityAddDescController () <UITextViewDelegate>
...
@implementation CityAddDescController
 ...
 *  @param message
 */
- (void) toJumoAddCity:(CityAddMessage *)message {
    
    // 取出 所有的控制器
    NSArray *subView = self.navigationController.viewControllers;
    for (CityAddController *cityAdd in subView) {
        if ([cityAdd isKindOfClass:[CityAddController class]]) {
            cityAdd.cityAddMessage       = message;
            [self.navigationController popToViewController:cityAdd animated:YES];
            break;
        }
    }
