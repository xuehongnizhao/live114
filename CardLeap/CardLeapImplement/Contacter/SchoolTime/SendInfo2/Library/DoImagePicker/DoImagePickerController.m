...
- (void)viewDidLoad
{
    ...
    self.dSelected = [[NSMutableDictionary alloc] init];
...
- (void)readAlbumList
{
    [ASSETHELPER getGroupList:^(NSArray *aGroups) {
       ...
       // NSLog(@"_btSelectAlbum.title:%@",_btSelectAlbum.titleLabel.text);
...
- (void)initBottomMenu
{
    ...
    {
        NSLog(@"_dselectCount=%ld",_dSelected.count);
       // _lbSelectCount.text = [NSString stringWithFormat:@"(%ld/%d)", _dSelected.count, (int)_nMaxCount];
...
- (IBAction)onSelectPhoto:(id)sender
{
    ...
    if (_nResultType == DO_PICKER_RESULT_UIIMAGE)
    {
        ...
        {
            NSLog(@"i=%d",i);
            UIImage* image=[ASSETHELPER getImageAtIndex:[aKeys[i] integerValue] type:ASSET_PHOTO_SCREEN_SIZE];
            //NSLog(@"getassetAtIndex:%@",[ASSETHELPER getAssetAtIndex:[aKeys[i] integerValue]]);
            if (image)
            {
                [aResult addObject:image];
            }
            else
            {
                NSLog(@"图片加入数组错误. akeys[%d]: %@",i,aKeys[i]);
            }            
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_nMaxCount > 1 || _nMaxCount == DO_NO_LIMIT_SELECT)
    {
		...
		if ((_dSelected[@(indexPath.row)] == nil) && (_nMaxCount > _dSelected.count))
		{
            ...
            NSLog(@"_dSelected[@(%ld)]:%@",indexPath.row,_dSelected[@(indexPath.row)]);
			[cell setSelectMode:YES];
		}
		else
		{
